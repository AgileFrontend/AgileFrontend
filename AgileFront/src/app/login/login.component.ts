    ...
    if (this.email && this.password) {
      ...
      signInWithEmailAndPassword(this.auth,this.email,this.password) // firebase authentication service call
        .then((usercredential) => {
          ...
          this.router.navigate(['homepage'],{ queryParams : {sessionToken : user.getIdToken()}}); //we redirect the user to the homepage with the session token as a query parameter
        })
        .catch((error) => { // Error handling alert

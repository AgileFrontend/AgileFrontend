...
export class SettingDrawerComponent {
  ...
  deleteConv(){
    ...
    this._bottomSheetRef.dismiss({event : "rename", data : this.nameToSend.value});
  ...
  updateName(){
    if(this.nameForm.valid){
      this.messagingService.updateConversationFromID(this.data,{conversationName : this.nameToSend.value})
      this._bottomSheetRef.dismiss({event : "rename", data : this.nameToSend.value});
